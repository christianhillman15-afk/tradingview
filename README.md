...
AI ensemble strategy showing its `P(up)` probability on each entry):

![Dashboard overview](docs/dashboard-overview.png)

The Trades tab — a full, color-coded blotter:

![Trade blotter](docs/dashboard-trades.png)
...
ai_futures_bot/
  ...
  indicators.py       Pure-Python SMA/EMA/RSI/MACD/Bollinger/ATR/VWAP/Donchian
  data.py             Bar model, CSV loader, synthetic OHLCV generator
  strategies/         Six strategies + base class + registry
  risk.py             Position sizing, daily-loss kill switch, drawdown halt
  portfolio.py        Position/Trade accounting, PnL, costs, slippage
  engine.py           Shared per-bar execution engine (backtest == live)
  backtester.py       Event-driven backtest driver
  live.py             Streaming paper trader (writes dashboard state)
  metrics.py          ROI, win rate, profit factor, drawdown, Sharpe, ...
